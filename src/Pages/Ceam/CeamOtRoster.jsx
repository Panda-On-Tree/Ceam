  ...
  return (
    <div className="ceam-roster-main">
      ...
      Download Roster Template
          ...
          {JSON.parse(localStorage.getItem("module_access"))?.back_day_roster_upload ? (
            <SlButton
              variant="primary"
              outline
              onClick={() => {
      ...
      <div className="table-ceam report-table roster-table  ">
        {rosterDataOT ? <MUIDataTable title="Reward Roster" data={rosterDataOT} columns={col} options={options}></MUIDataTable> : <p className="no-data">No Data Found For This Month</p>}
      </div>
      ...
      <SlDialog label="Update/Add Single Reward" open={openUpSignleOt} onSlRequestClose={() => setOpenUpSignleOt(false)}>
        <div className="update-ot-single-main">
          ...
          >
            ...
            {otList?.map((item, i) => {
              return (
                <SlMenuItem key={`${i}ot`} value={item.shift_character}>
                  {item.shift_character}
                </SlMenuItem>
              );
            })}
