					...
					return (
						<div className="edit-button-main">
							{!empData[dataIndex].aadhar_verify_flag ? (
								...
								<SlTag variant="success" size="small" className="tag-row" onClick={(e) => {}} style={{ cursor: 'not-allowed', pointerEvents: 'none' }}>
									Verified
								</SlTag>
							)}
						</div>
			...
			<SlDialog style={{ '--width': '80vw' }} label="Add Employee" open={openAddEmp} onSlRequestClose={() => setOpenAddEmp(false)}>
				<div className="add-emp-inputs-main">
					...
					<SlSelect
						...
						onSlChange={(e) => {
							setSingleEmployeeUpload({ ...singleEmployeeUpload, department: e.target.value });
						}}>
						{deptList?.map((item, i) => {
							return (
					...
					<SlSelect
						...
						onSlChange={(e) => {
							setSingleEmployeeUpload({ ...singleEmployeeUpload, category: e.target.value });
						}}>
					...
					<SlSelect
						...
						onSlChange={(e) => {
							...
							setSingleEmployeeUpload({ ...singleEmployeeUpload, base_location: e.target.value });
						}}>
					...
					<SlSelect
						...
						onSlChange={(e) => {
							setSingleEmployeeUpload({ ...singleEmployeeUpload, division: e.target.value });
						}}>
			...
			<SlDialog label="Update" open={openUpdateEmp} onSlRequestClose={() => setOpenUpdateEmp(false)}>
				<div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
					<SlSelect
						...
						onSlChange={(e) => {
							getDivision(e.target.value);
							setEmpUpdateData({ ...empUpdateData, location: e.target.value });
						}}>
						{plantList?.map((item, i) => {
							return (
								<SlMenuItem key={`${i}plant`} value={item.plant}>
					...
					<SlSelect
						...
						label="Select Division"
						onSlChange={(e) => {
							setEmpUpdateData({ ...empUpdateData, division: e.target.value });
						}}>
						{divisionList?.map((item, i) => {
							return (
								<SlMenuItem key={`${i}divisionu`} value={item.division}>
